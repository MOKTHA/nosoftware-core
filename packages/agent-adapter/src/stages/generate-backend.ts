  ...
  private async generateBackendInSandbox(input: GenerationStageInput): Promise<void> {
    ...
    const routeCode = await callOpenRouter({
      ...
      systemPrompt: [
        ...
        'EVERY route handler MUST wrap database operations in try/catch.',
        'On error, return NextResponse.json({ error: message }, { status: 500 }).',
        'GET handlers: support both list (no params) and single-item (via searchParams id).',
        'POST handlers: parse body with try/catch, validate required fields, return 400 on bad input.',
        'PATCH handlers: parse body, use eq() for WHERE, return updated record.',
        'DELETE handlers: use eq() for WHERE, return { success: true }.',
        '',
        '## File format',
        'Create one file per entity: app/api/<entity>/route.ts',
        'For single-item routes: app/api/<entity>/[id]/route.ts',
